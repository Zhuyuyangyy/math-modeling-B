...
class Robot:
    ...
    def run(self):
        ...
        # Q3: nearest square-lattice vertex <=1200/sqrt(2)<1000.
        # Q4: every point lies in a 650m square; all four vertices are
        # within 650*sqrt(2)<1000. Any closed half-plane through that
        # point contains at least one of these four vertices.
        if self.problem == 3:
            sites = [(x,y) for x in range(-2400,2401,1200) for y in range(-2400,2401,1200)
                     if math.hypot(x,y) <= 2700]
        else:
            sites = [(x,y) for x in range(-1950,1951,650) for y in range(-1950,1951,650)]
            sites.append((0,0)) if (0,0) not in sites else None
        ...
        summary = dict(mode='practice', problem=self.problem, label=self.label,
...
if __name__ == '__main__':
    ...
    from practice_robot import practice_preflight
